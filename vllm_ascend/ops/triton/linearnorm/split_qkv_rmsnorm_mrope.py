...
def split_qkv_rmsnorm_mrope_kernel(
    ...
    for index in range(loop_num):
        ...
        if gate_size > 0:
            ...
            in_q_tensor = extension.extract_slice(
                in_q_gate_tensor,
                offsets=(0, 0),
                sizes=(num_q_heads, head_size),
                strides=(1, 1),
            )
            in_gate_tensor = extension.extract_slice(
                in_q_gate_tensor,
                offsets=(0, head_size),
                sizes=(num_q_heads, head_size),
                strides=(1, 1),
        ...
        if IS_PARTIAL_ROPE:
            orig_qk = extension.extract_slice(
                q_normalized,
                offsets=(0, 0),
        ...
        # k-mrope
        y1 = extension.extract_slice(
            k_normalized,
            offsets=(0, 0),
            sizes=(num_kv_heads, half_rope_dim),
            strides=(1, 1),
        )
        y2 = extension.extract_slice(
            k_normalized,
            offsets=(0, half_rope_dim),
            sizes=(num_kv_heads, half_rope_dim),
            strides=(1, 1),
        ...
        if IS_PARTIAL_ROPE:
            q_normalized = extension.insert_slice(
                q_normalized,
                roped_q,
                offsets=(0, 0),
                sizes=(num_q_heads, rope_dim),
            ...
            k_normalized = extension.insert_slice(
                k_normalized,
                roped_k,
                offsets=(0, 0),
                sizes=(num_kv_heads, rope_dim),
