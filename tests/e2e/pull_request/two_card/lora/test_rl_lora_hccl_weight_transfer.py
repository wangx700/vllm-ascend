...
#
"""End-to-end test for verl-style RL LoRA tensor synchronization over HCCL.

Topology (requires 2 NPUs):

* Logical NPU 0 runs vLLM with the base model and ``rl_adapter``.
* Logical NPU 1 represents the trainer holding live LoRA tensors.

The vLLM OpenAI layer uses a static adapter only to register ``rl_adapter`` and
its stable integer id. Alice and Bob are then both transferred from trainer NPU
memory over one reused HCCL group and installed by a test-only worker extension,
without writing or reloading an adapter checkpoint.
"""
...
class LoRAHCCLWorkerExtension:
    """Receive LoRA tensors over the configured HCCL weight-transfer engine."""

    def update_lora_weights_from_hccl(
        self,
        ...
        def collect_bucket(weights: list[tuple[str, torch.Tensor]]) -> None:
            nonlocal received_bytes
            for name, tensor in weights:
                if name in lora_tensors:
                    raise ValueError(f"Received duplicate LoRA tensor {name!r}")
                # Packed HCCL buffers are reused. Keep adapter-owned CPU storage.
        ...
        with torch.device(self.device):
            self.weight_transfer_engine.receive_weights(update_info, collect_bucket)
        ...
        # Match verl's adapter-mode update: replace one stable logical adapter.
...
# These fields must stay fixed while live LoRA tensors are replaced.
...
def _load_trainer_lora(source_adapter: Path) -> dict[str, torch.Tensor]:
    """Load a checkpoint once to simulate live LoRA parameters on trainer NPU."""
    source_weights = source_adapter / "adapter_model.safetensors"
    trainer_device = f"npu:{TRAINER_DEVICE_INDEX}"
...
def _collect_lora_metadata(tensors: dict[str, torch.Tensor]) -> dict:
    return {
        "names": list(tensors),
        "dtype_names": [str(tensor.dtype).removeprefix("torch.") for tensor in tensors.values()],
        "shapes": [list(tensor.shape) for tensor in tensors.values()],
        "packed": True,
...
def _transfer_lora(
    ...
    update_info = _collect_lora_metadata(tensors)
    expected_bytes = sum(
    ...
    update_thread = _BackgroundPost(
        ...
        "collective_rpc",
        json_body={
            "method": "update_lora_weights_from_hccl",
    ...
    assert update_thread.response is not None
    result = update_thread.response.json()["results"][0]
...
@pytest.mark.skipif(
    ...
    reason="RL LoRA HCCL transfer test requires at least 2 NPUs.",
)
def test_rl_lora_hccl_weight_transfer_and_hot_update():
    from tests.e2e.conftest import RemoteOpenAIServer
    ...
    _log("starting vLLM on NPU 0; trainer uses NPU 1")
    with RemoteOpenAIServer(
        ...
        group = _init_hccl_group(server)

        _log("step 1: transferring live Alice LoRA tensors over HCCL")
        alice_result = _transfer_lora(server, group, alice_lora_path, alice_config)
        alice_output = _generate_identity(client, LORA_NAME)
        _log(f"Alice output: {alice_output!r}")

        _log("step 2: transferring live Bob LoRA tensors over HCCL")
        bob_result = _transfer_lora(server, group, bob_lora_path, bob_config)
